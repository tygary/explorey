...
class GhostPrinter(object):
    ...
    def __create_ghost(self, ghost):
        ...
        pdf.multi_cell(0, 10, f"{ghost.name}", align="C")
        pdf.set_font("helvetica", "", 12)
        pdf.cell(90, 4, ln=1)
        pdf.multi_cell(0, 6, f"Origin: {ghost.origin}", align="L")
        pdf.multi_cell(0, 6, f"Amount Fulfilled: {ghost.origin}", align="L")
        pdf.multi_cell(0, 6, f"Used: {ghost.origin}", align="L")
        pdf.multi_cell(0, 6, f"Previous Owner: {ghost.origin}", align="L")
        pdf.multi_cell(0, 6, f"Reason Discarded: {ghost.origin}", align="L")
        pdf.multi_cell(0, 6, f"Date of Disposal: {ghost.origin}", align="L")
        pdf.multi_cell(0, 6, f"Fact One: {ghost.origin}", align="L")
        pdf.multi_cell(0, 6, f"Fact Two: {ghost.origin}", align="L")
